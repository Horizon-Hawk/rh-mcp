...
def main():
    DATA_FILE = "backtest_data/mes_15min_continuous.csv"
    bars = load_bars(DATA_FILE)
    print(f"Loaded {len(bars)} bars from {DATA_FILE}")
    # NOTE: POINT_VALUE inherited from _run_mnq_orb_backtest = $2/pt (MNQ).
    # MES is $5/pt. Dollar P&L below is in MNQ-equivalent units; multiply
    # totals by 2.5 to get true MES dollars. Win rate / PF / R-multiples
    # are point-value-agnostic — those are the comparable metrics.
    daily = load_daily("backtest_data/mes_1d_continuous.csv")

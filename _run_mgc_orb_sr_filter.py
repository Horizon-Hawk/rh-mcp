...
def main():
    DATA_FILE = "backtest_data/mgc_15min_continuous.csv"
    bars = load_bars(DATA_FILE)
    print(f"Loaded {len(bars)} bars from {DATA_FILE}")
    # NOTE: POINT_VALUE inherited from MNQ ($2/pt). MGC = $10/pt.
    # Dollar P&L is MNQ-equivalent; multiply by 5x for true MGC dollars.
    # PF / WR / R-multiples are point-value-agnostic.
    daily = load_daily("backtest_data/mgc_1d_continuous.csv")

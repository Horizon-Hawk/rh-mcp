"""Mid-cap universe: Finviz cap_mid ($2B-$10B) + price>$5 + vol>500K."""
import re
import sys
import time
...
FILTERS = "cap_mid,sh_price_o5,sh_avgvol_o500"
...
def main():
    ...
    print(f"\nFetched {len(all_t)} mid-cap tickers")
    print("First 20:", " ".join(all_t[:20]))
    print("Last 20:", " ".join(all_t[-20:]))

    lines = [
        "# Mid-cap universe (Finviz cap_mid: $2B-$10B, price>$5, avg vol>500K)",
        f"# Count: {len(all_t)}",
        "",
    ]
    for i in range(0, len(all_t), 10):
        lines.append(" ".join(all_t[i:i + 10]))
    out = Path("C:/Users/algee/TraderMCP-RH/mid_cap_universe.txt")
    out.write_text("\n".join(lines) + "\n")

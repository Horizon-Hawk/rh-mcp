"""MGC RSI(2) backtest — port of MNQ Phase 3.5 framework to gold daily.

Pattern scan result that prompted this:
  MGC RSI(2)<5 daily → LONG, 5d hold:  WR 57%, mean +0.568%, N=249 (3.5y)
...
Strategy variants tested (escalating filter rigor):
  ...
  V3: Long+Short          →  V2 + RSI>95 short branch (testing MGC short edge)
  V4: Bare RSI(2)>95 short →  no filters

For each: trades, win rate, PF, total $, max DD, annualized.

Data: backtest_data/mgc_1d_continuous.csv (1041 RTH-aggregated daily bars,
Oct 2022 → May 2026).
"""
...
POINT_VALUE = 10.0  # $/pt MGC = $10
DATA_FILE = Path("backtest_data/mgc_1d_continuous.csv")
...
def main():
    bars = load_bars()
    print(f"MGC daily bars: {len(bars)}  ({bars[0].date} → {bars[-1].date})")
    print(f"Years: {len(bars) / 252.0:.2f}\n")

    print("=" * 138)
    print("MGC RSI(2) STRATEGY VARIANTS — daily, 5d max hold, RSI-revert exit")
    print("=" * 138)
